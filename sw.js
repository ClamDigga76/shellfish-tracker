...
const CACHE_NAME = `shellfish-tracker-v30-${CACHE_VERSION}`;
...
self.addEventListener("activate", (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(keys.map((k) => (k.startsWith("shellfish-tracker-v30-") && k !== CACHE_NAME) ? caches.delete(k) : Promise.resolve()));
