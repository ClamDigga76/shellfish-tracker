...
const HOME_FREE_KPI_DETAIL_CONFIG = Object.freeze({
  ...
  ppl: Object.freeze({
    ...
    freeChartKeys: Object.freeze([
      ...
      Object.freeze({ key: "pplPriceRangeByTrip", title: "Trips by price per pound range, only when the chart is actually price-per-pound ranges.", context: "Shows how many trips landed in each price-per-pound range." })
    ]),
...
export function createReportsMetricDetailSeam(deps){
  ...
  const getPplFormulaText = ({ metricKey, surface = "default" } = {})=> {
    if(metricKey !== "ppl") return "";
    if(surface === "short") return "Average price/lb = total pay ÷ total pounds.";
