...
function parseBackupFileForRestore(file){
  return readTextFile(file).then((txt)=>{
    ...
    try{
      ...
      throw new Error(`Invalid JSON: ${String(e?.message || e || "Parse failed")}`);
    }

    const normalizedResult = normalizeBackupPayload(raw);
    if(!normalizedResult.ok){
      throw new Error(normalizedResult.errors.join("\n"));
    }
...
function openRestorePreviewModal(preview){
  return new Promise((resolve)=>{
    ...
    const warningHtml = (preview.warnings && preview.warnings.length)
      ? `<div class="modalErr" style="display:block;margin-top:10px">
          <b>Warnings</b>
          <ul style="margin:8px 0 0 16px;padding:0">${preview.warnings.map(w=>`<li>${escapeHtml(String(w || ""))}</li>`).join("")}</ul>
        </div>`
  ...
  return new Promise((resolve)=>{
    ...
    openModal({
      ...
      onOpen: ()=>{
        ...
        document.getElementById(skipId)?.addEventListener("click", ()=>{
          closeModal();
          resolve("skip");
        });
...
function renderSettings(opts={}){
  ...
  try{ settingsListManagement.bindListMgmtHandlers(); }catch(_){ }
