...
import { uid, toCSV, downloadText, formatMoney, formatDateMDY, computePPL, to2, parseMDYToISO, parseNum, parseMoney, likelyDuplicate, normalizeKey, escapeHtml } from "./utils_007N.js";

const VERSION = "ESM-007N";

// Boot flag for index.html watchdog
try{ window.__APP_BOOTED = true; }catch{}

// Let the bootloader know the module executed.
...
function navTo(view){
  // Used for simple in-app back navigation (Settings/Reports/Help/About)
  state.navReturn = state.view || "home";
  state.view = view;
  saveState();
  render();
}

function navBack(fallback="home"){
  const target = state.navReturn || fallback;
  state.navReturn = "";
  state.view = target;
  saveState();
  render();
...
function loadState(){
  try{
    ...
    return {
      ...
      view: p?.view || "home",
      // Used for simple in-app back navigation (Help/About/etc)
      navReturn: p?.navReturn || "",
      filter: p?.filter || "YTD",
      settings: p?.settings || {},
      areas: p?.areas || []
    };
  }catch{
    return { trips: [], view: "home", navReturn: "", filter: "YTD", settings: {}, areas: [] };
  }
}

function go(view, returnTo=""){
  // Lightweight navigation helper: remembers where to go back (if provided)
  state.navReturn = returnTo || state.navReturn || "";
  state.view = view;
  saveState();
  render();
}

function goHelp(from=""){
  state.navReturn = from || state.view || "home";
  state.view = "help";
  saveState();
  render();
}

function goAbout(from=""){
  state.navReturn = from || state.view || "home";
  state.view = "about";
  saveState();
  render();
...
function renderHome(){
    ...
    return `
      <div class="trip" data-id="${t?.id||""}" role="button" tabindex="0">
        ...
        <div class="trip-meta">
          ...
          <span class="pill">Total: <b>${formatMoney(amt)}</b></span>
  ...
  app.innerHTML = `
    <div class="card">
      ...
      <div class="hint">Log a trip using Live Text copy/paste. <b>Review → Confirm</b> is the only way a trip is saved.</div>
    </div>
  ...
  app.querySelectorAll("button.chip").forEach(btn=>{
    ...
    document.getElementById("reports").onclick = ()=>{ state.lastAction="nav:reports"; navTo("reports"); };
    document.getElementById("help").onclick = ()=>{ state.lastAction="nav:help"; navTo("help"); };

  document.getElementById("settings").onclick = ()=>{ state.lastAction="nav:settings"; navTo("settings"); };
...
const topAreas = (()=>{
  ...
  app.innerHTML = `
    <div class="card">
      ...
      <div class="hint">Enter the check info. <b>Nothing auto-pastes</b> — tap <b>Paste → Review</b> when you're ready. Date should be harvest date (MM/DD/YYYY).</div>
    </div>
    ...
    <div class="card">
      <div class="form">
        <div class="field">
          ...
          <div class="hint">After copying receipt text with Live Text, tap <b>Paste → Review</b>. You can edit fields before saving.</div>
...
<select class="select" id="t_area">
        ...
        <div class="actions">
          <button class="btn primary" id="saveTrip">Review</button>
          <button class="btn" id="cancelTrip">Cancel</button>
          <button class="btn danger" id="clearDraft">Clear Draft</button>
        </div>
      </div>
    </div>
  `;
...
function renderEditTrip(){
  ...
  app.innerHTML = `
    <div class="card">
      <div class="row" style="justify-content:space-between;align-items:center">
        <button class="smallbtn" id="backHome">← Back</button>
        <b>Edit Trip</b>
        <span class="muted small" id="editDirty"></span>
      </div>
      <div class="hint">Make changes, then tap <b>Save</b>.anges when finished. Delete removes the trip from this device.</div>
    </div>
  ...
  const elDirty = document.getElementById("editDirty");

  const original = {
    date: (elDate?.value||"").trim(),
    dealer: (elDealer?.value||"").trim(),
    pounds: (elPounds?.value||"").trim(),
  ...
  const setDirty = ()=>{
    if(dirty) return;
    dirty = true;
    if(elDirty) elDirty.textContent = "Unsaved";
  };
  ...
  document.getElementById("backHome").onclick = ()=>{ if(!confirmDiscard()) return; goHome(); };

  const hBtn = document.getElementById("openHelp");
  if(hBtn) hBtn.onclick = ()=>{ state.lastAction="nav:help"; goHelp("settings"); };

  const aBtn = document.getElementById("openAbout");
  if(aBtn) aBtn.onclick = ()=>{ state.lastAction="nav:about"; goAbout("settings"); };
  ...
  document.getElementById("cancelEdit").onclick = ()=>{ if(!confirmDiscard()) return; goHome(); };
...
function renderSettings(){
  ...
  app.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="muted small" style="margin-top:10px">Reset removes all trips and settings on this device.</div>
      <div class="row" style="margin-top:12px">
    ...
    <div class="card">
      <b>Help & About</b>
      <div class="sep"></div>
      <div class="muted small" style="margin-top:10px">Need a refresher or want to report a bug? Use Help, or copy debug info to share for troubleshooting.</div>
      <div class="row" style="margin-top:12px">
...
function renderHelp(){
  ...
  document.getElementById("backHome").onclick = ()=>{
    const dest = state.navReturn || "home";
    state.navReturn = "";
    state.view = dest;
    state.lastAction = "nav:back";
    saveState();
    render();
  };
...
function renderAbout(){
  ...
  document.getElementById("backSettings").onclick = ()=> navBack("home");
    state.navReturn = "";
    state.view = dest;
    state.lastAction = "nav:back";
    saveState();
...
try{ render(); window.__APP_BOOTED = true; }catch(err){ setBootError(err?.message || err); throw err; }
